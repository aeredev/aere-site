...
import { Search, X, ChevronDown, ExternalLink, BookOpen } from 'lucide-react'
...
const SEX_CONFIG = {
  male:   { label: 'Male only',   color: 'var(--color-blue)', bg: '#eef1fc' },
  female: { label: 'Female only', color: 'var(--color-accent)', bg: 'var(--color-accent-tint)' },
  both:   { label: 'All',         color: 'var(--color-muted)', bg: 'var(--color-surface-2)' },
} as const

type SexFilter = 'all' | 'male' | 'female' | 'both'
...
function BiomarkerCard({
  ...
  return (
    ...
    >
      ...
      >
        {/* Row 1: name + badges + last updated */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            <span style={{ fontSize: 11, color: 'var(--color-muted)' }}>
              Updated {biomarker.last_updated}
            </span>
        ...
        <div style={{ display: 'flex', gap: 10 }} className="range-row">
          <RangeCell label="Standard lab range" display={biomarker.standard_lab_range.display} />
          <RangeCell label="Aere optimal" display={biomarker.aere_optimal.display} isOptimal />
        </div>
      </button>
      ...
      {isExpanded && (
        <div style={{
          ...
          {/* Citations — first visible, rest behind toggle */}
          {biomarker.citations.length > 0 && (
            <div>
              ...
              {biomarker.citations.length > 1 && !showAllCitations && (
                <button
                  onClick={(e) => { e.stopPropagation(); setShowAllCitations(true) }}
                  style={{
                ...
                >
                  Show all {biomarker.citations.length} citations
                </button>
              )}
              {showAllCitations && biomarker.citations.length > 1 && (
                <button
                  onClick={(e) => { e.stopPropagation(); setShowAllCitations(false) }}
                  style={{
                    background: 'none', border: 'none', cursor: 'pointer',
...
export default function ScienceClientV2({ biomarkers, categories }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return biomarkers.filter(b => {
      ...
      const matchSex = sexFilter === 'all' || b.sex_specific === sexFilter
  ...
  }, [biomarkers, selectedCategory, searchQuery, sexFilter])
  ...
  return (
    <section id="reference" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 100px' }}>
      ...
      {/* Section header */}
      <div style={{ textAlign: 'center', paddingTop: 88, marginBottom: 40 }}>
        <p style={{
        ...
        <p style={{ fontSize: 16, color: 'var(--color-muted)', maxWidth: 520, margin: '0 auto' }}>
          Search by name or alias. Filter by category or sex. Click any biomarker to read the evidence.
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginBottom: 48 }}>
        {([
          ...
          { value: 'male', label: 'Male-specific' },
          { value: 'female', label: 'Female-specific' },
        ] as const).map(({ value, label }) => (
          <button
            key={value}
            onClick={() => setSexFilter(value)}
            className={`sex-pill${sexFilter === value ? ' active' : ''}`}
          >
            {label}
          </button>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '224px 1fr', gap: 32, alignItems: 'start' }} className="ref-layout">
        ...
        <div>
          <div style={{ fontSize: 13, color: 'var(--color-muted)', marginBottom: 16, fontWeight: 500 }}>
            ...
            {sexFilter !== 'all' && (
              <span> ({sexFilter === 'both' ? 'shared' : `${sexFilter}-specific`})</span>
            )}
          </div>
          ...
          {filtered.length === 0 ? (
            <div style={{
              ...
              <div style={{ fontSize: 14, color: 'var(--color-muted)' }}>
                ...
                <button
                  onClick={() => { setSearchQuery(''); setSelectedCategory('all'); setSexFilter('all') }}
